...
def get_pruner(dataset, method='datawhisperer'):
    if method == 'datawhisperer':
        ...
        return HeuristicPruner
    else:
        raise ValueError(f"Unknown method: {method}. Supported methods: datawhisperer, random, heuristic")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default='datawhisperer', help="Selecting pruning method: datawhisperer, random, heuristic")
    ...
    # Random & Heuristic specific arguments
    parser.add_argument("--keep_ratio", type=float, default=0.5, help="Proportion of data to keep (for random and heuristic methods)")
    parser.add_argument("--heuristic", type=str, default='length', help="Heuristic strategy: length, answer_length, quality_score, dedup")
    parser.add_argument("--keep_long", type=bool, default=True, help="For heuristic: keep long samples (True) or short samples (False)")
    parser.add_argument("--similarity_threshold", type=float, default=0.8, help="For dedup heuristic: Jaccard similarity threshold (0.0-1.0)")
