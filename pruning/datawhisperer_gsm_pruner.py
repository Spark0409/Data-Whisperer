...
class DataWhisperer_GSM_Pruner(Pruner):
    ...
    def extract_predictions(self,responses_section: str) -> List[float]:
        predictions = []
        # Match the "####" line and capture the number, allowing commas and decimals
        matches = re.findall(r'####\s*([\d,]+(?:\.\d+)?)', responses_section)

        for match in matches:
            # Remove commas from the matched number
            cleaned_number = match.replace(",", "")
            try:
                # Convert to float (works for both integers and decimals)
                predictions.append(float(cleaned_number))
            except ValueError:
                # Skip invalid conversions (unlikely unless the input is malformed)
                continue
    ...
    def predict_batch(
        ...
        for demonstrations, val_samples in zip(batch_demonstrations, batch_val_samples):
            ...
            for i, sample in enumerate(val_samples):
                all_texts.append(
                    f'Question {i + 1}: "{sample["question"]}"'
                )       
    ...
    def _evaluate_single_fold(
        ...
        while train_pointer < len(train_batches):
            ...
            for predictions, val_samples, selected_indices, attention_scores in zip(
                ...
                references = [self.extract_answer(sample["answer"]) for sample in val_samples]
